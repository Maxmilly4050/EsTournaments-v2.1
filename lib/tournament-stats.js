...
export class TournamentStatsService {
  ...
  calculateStats(userId, participations, matches) {
    ...
    let winRate = 0
    if (totalGames > 0) {
      const rawWinRate = (wins.length / totalGames) * 100
  ...
  calculateGameStats(participations, matches, userId) {
    ...
    matches.forEach((match) => {
      ...
      if (gameStats[game] && match.status === "completed" && match.winner_id) {
        ...
        if (total > 0) {
          const rawWinRate = (gameStats[game].wins / total) * 100
          // Apply bounds checking to ensure win rate stays within 0-100%
          gameStats[game].winRate = Math.round(Math.max(0, Math.min(100, rawWinRate)))
        } else {
          gameStats[game].winRate = 0
        }
      }
    })
