...
function GoogleSignInButton() {
  ...
  const handleGoogleSignIn = async () => {
    setIsLoading(true)
    console.log("[v0] Login form - Starting Google sign-in")
    try {
      const result = await signInWithGoogle()
      console.log("[v0] Login form - Google sign-in result:", result)
      if (result?.url) {
...
export default function LoginForm() {
  ...
  const handleSubmit = async (event) => {
    ...
    console.log("[v0] Login form - Starting sign-in process")
    console.log("[v0] Login form - Email:", formData.get("email"))

    try {
      const result = await signIn(null, formData)
      console.log("[v0] Login form - Sign-in result:", result)
      setState(result)
    } catch (error) {
      console.log("[v0] Login form - Sign-in error:", error)
